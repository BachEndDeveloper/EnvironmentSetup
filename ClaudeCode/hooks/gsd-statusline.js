...
function runStatusline() {
  ...
  try {
    ...
    // Effort suffix appended to the model segment. The `effort` block is only
    // present when the current model supports reasoning effort (e.g. Opus 4.7).
    // Color rises with level so the statusline reflects how hard the model is
    // currently allowed to think.
    let effortSuffix = '';
    ...
    if (effortLevel) {
      ...
      effortSuffix = ` ${color}(${effortLevel})\x1b[0m`;
    }
    ...
    if (middle) {
      process.stdout.write(`${gsdUpdate}\x1b[2m${model}\x1b[0m${effortSuffix} │ ${middle} │ \x1b[2m${dirname}\x1b[0m${ctx}${lastCmdSuffix}`);
    } else {
      process.stdout.write(`${gsdUpdate}\x1b[2m${model}\x1b[0m${effortSuffix} │ \x1b[2m${dirname}\x1b[0m${ctx}${lastCmdSuffix}`);
    }
  } catch (e) {
...
function renderStatusline(data) {
  ...
  if (middle) {
    return `\x1b[2m${model}\x1b[0m${effortSuffix} │ ${middle} │ \x1b[2m${dirname}\x1b[0m${lastCmdSuffix}`;
  }
  return `\x1b[2m${model}\x1b[0m${effortSuffix} │ \x1b[2m${dirname}\x1b[0m${lastCmdSuffix}`;
}
